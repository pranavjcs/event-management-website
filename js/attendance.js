...
(() => {
  ...
  async function getEventAttendanceApi(eventId) {
    const result = await apiRequest(`/attendance/${encodeURIComponent(eventId)}`);
    return result.data || {};
  }

  async function getEventAttendanceCountApi(eventId) {
    const attendance = await getEventAttendanceApi(eventId);
    return Object.values(attendance).filter((value) => value === true).length;
  }

  async function markAttendanceApi(eventId, registrationId, attended) {
    await apiRequest(`/attendance/${encodeURIComponent(eventId)}/${encodeURIComponent(registrationId)}`, {
      method: "PUT",
      body: JSON.stringify({ attended: !!attended })
    });
  }

  async function loadEventSelectorApi() {
    const selector = document.getElementById("attendance-event");
    if (!selector) return;
    ...
    try {
      ...
      selector.onchange = () => {
        if (selector.value) {
          displayEventRegistrantsApi(selector.value);
        } else {
          const container = document.getElementById("attendance-container");
  ...
  async function displayEventRegistrantsApi(eventId) {
    const container = document.getElementById("attendance-container");
    if (!container) return;

    try {
      const [eventResult, registrationsResult, attendanceMap] = await Promise.all([
        apiRequest(`/events/${encodeURIComponent(eventId)}`),
        apiRequest(`/registrations?eventId=${encodeURIComponent(eventId)}`),
        getEventAttendanceApi(eventId)
      ]);

      const event = eventResult.data;
      const eventRegistrations = registrationsResult.data;
      ...
      if (eventRegistrations.length === 0) {
        ...
        eventRegistrations.forEach((registration) => {
          const isAttended = attendanceMap[registration.id] === true;
          html += `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background: var(--surface); border-radius: 8px; border: 1px solid #e5e7eb;">
              <div>
                <p style="font-weight: 600; margin: 0;">${registration.name}</p>
                <p style="font-size: 12px; color: var(--muted); margin: 4px 0;">${registration.email}</p>
              </div>
              <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                <input
                  type="checkbox"
                  ${isAttended ? "checked" : ""}
                  onchange="window.attendance.markAttendance('${eventId}', '${registration.id}', this.checked).then(() => window.attendance.displayAttendanceSummary())"
                  style="width: 20px; height: 20px; cursor: pointer;"
                />
                <span>Attended</span>
              </label>
      ...
      container.innerHTML = `<p>${error.message || "Unable to load attendance."}</p>`;
    }
  }

  async function displayAttendanceSummaryApi() {
    const tbody = document.getElementById("attendance-summary-tbody");
    if (!tbody) return;

    try {
      const result = await apiRequest("/attendance-summary");
      ...
      summary.forEach((item) => {
        ...
        row.innerHTML = `
          ...
          <td>${item.attended}</td>
          <td><strong>${item.attendancePercent}%</strong></td>
        `;
        tbody.appendChild(row);
      });
      ...
      if (summary.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="4" style="text-align: center; color: var(--muted);">No events available.</td>';
        tbody.appendChild(row);
      }
    } catch (error) {
      tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--muted);">${error.message || "Unable to load summary."}</td></tr>`;
    }
  }

  async function loadAttendancePageApi() {
    const admin = window.auth && window.auth.getCurrentAdmin
      ? window.auth.getCurrentAdmin()
      : null;
    ...
    await loadEventSelectorApi();
    await displayAttendanceSummaryApi();
  }

  window.attendance = {
    ...window.attendance,
    loadAttendancePage: loadAttendancePageApi,
    displayEventRegistrants: displayEventRegistrantsApi,
    displayAttendanceSummary: displayAttendanceSummaryApi,
    markAttendance: markAttendanceApi,
    getEventAttendanceCount: getEventAttendanceCountApi,
    getEventAttendance: getEventAttendanceApi
  };
})();
